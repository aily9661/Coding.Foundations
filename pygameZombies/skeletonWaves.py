...
myDNote = sa.WaveObject.from_wave_file("DNote.wav")
myFSNote = sa.WaveObject.from_wave_file("FSNote.wav")
myANote = sa.WaveObject.from_wave_file("ANote.wav")
prevDist = 0.0
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                if distanceWrists < 0.1 and playDNote:
                    myDNote.play()
                    playDNote = False
                elif distanceWrists > 0.4 and not playDNote:
                    myDNote.play().stop()
                    playDNote = True

                if distanceWristToKneeLeft < 0.1 and playFSNote:
                    myFSNote.play()
                    playFSNote = False
                elif distanceWristToKneeLeft > 0.4 and not playFSNote:
                    myFSNote.play().stop()
                    playFSNote = True

                if distanceWristToKneeRight < 0.1 and playANote:
                    myANote.play()
                    playANote = False
                elif distanceWristToKneeRight > 0.4 and not playANote:
                    myANote.play().stop()
                    playANote = True
