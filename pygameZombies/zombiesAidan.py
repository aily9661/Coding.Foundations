    ...
    def update(self):
        ...
        if self.diesAnimation.is_animating:
            ...
            if self.diesAnimation.spriteListIdx == 0:
                self.kill()
        elif self.walkingAnimation.is_animating:
            self.image = self.walkingAnimation.getNextImage()
...
class NPC(Player):
    def __init__(self,pos_x,pos_y,color,player):
        ...
        #PC09: Add an AnimationData() property for the
        #zombies dies animation
        #self.diesAnimation = AnimationData("skeldies_",9,color)
    ...
    def update(self):
        ...
        #PC09 we need to have two branches in our NPC(Player)
        #class. If the diesAnimation is_animating we need to call
        #getNextImage() on the diesAnimation object until the
        #spriteListIdx variable equals 0, then we need to
        #call "self.kill()" to remove the sprite that was hit
        #with a projectile
        #<if diesAnimation is animating, run the animation once
        #then kill the sprite, which will remove it from the sprite
        #group
        if self.diesAnimation.is_animating:
            self.image = self.diesAnimation.getNextImage()
            if self.diesAnimation.spriteListIdx == 0:
...
next_shot_right_time = 0
next_shot_left_time = 0
shot_interval_time = 150
max_shots = 5
...
while True:
    ...
    if len(shots_group) <= max_shots:
        if shootLeft != 0:
            if time_now > next_shot_left_time:
                shot = Shot(player.rect.centerx,player.rect.centery,True)
                shots_group.add(shot)
                next_shot_left_time = time_now + shot_interval_time
       
    shootRight = keystate[pygame.K_l]
    time_now = pygame.time.get_ticks()
    if len(shots_group) <= max_shots:
        if shootRight != 0:
            if time_now > next_shot_right_time:
                shot = Shot(player.rect.centerx,player.rect.centery,False)
                shots_group.add(shot)
                next_shot_right_time = time_now + shot_interval_time
