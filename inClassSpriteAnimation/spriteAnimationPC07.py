...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, pos_x,pos_y,color):
        ...
        for i in range(8):
            ...
            self.jumpSprites.append(self.getColorImage(myImage,color))  
    ...
    def runAnimationJump(self):
        #increment self.current_sprite if self.is_animating is True
        if self.is_jumping:
            self.current_jumpSprites += 0.25

            if self.current_jumpSprites >= len(self.jumpSprites):
                self.current_jumpSprites = 0
                self.is_jumping = False

            #set self.image which is what is drawn when the moving_sprites.draw(screen)
            #call is made below in the main while loop. 
            self.image = self.jumpSprites[int(self.current_jumpSprites)]
...
player = Player(0,100,"red")
playerTwo = Player(0,300,"green")
